...
### 子技能自动调用规则（实施阶段，强制）
- 进入执行阶段后，先根据任务类型调用对应子技能；如触发多个类型，按以下优先规则依次调用（仅限当前任务范围，禁止顺手扩展）：
  - DB/迁移/表结构：优先 db-migrate
  - 新增/修改 API：优先 api-contract
  - 测验生成/提交/难度/回调：优先 quiz-engine
  - 画像更新/弱项统计/推荐：优先 learner-profile
  - 新增关键链路：必须调用 smoke-maintainer 更新 dev_smoke
  - 门禁失败/报错：优先 bug-triage
...
### TASKS 状态更新（强制）
- 合并前不得将任务标记为 [x]；在 run_next_task 结束时保持 [~]
- 仅在 merge_task 完成并由用户确认后，才将任务从 [~] 改为 [x]
...
- 合并命令执行情况
- 合并后 smoke/验证情况（如执行）
- 合并成功且用户确认后，更新 TASKS.md 将对应任务标记为 [x]
