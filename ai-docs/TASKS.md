...
当满足以下任一条件时，允许 Agent 自动分析仓库�?CURRENT/TASKS，并�?TASKS.md 末尾“追加”新任务�?
- 任务队列剩余未完成任�?<= 3 �?
- 刚完成一个里程碑（里程碑验收记录写完�?
...
### 里程碑列�?
- Milestone A（后端测验闭环）：覆�?P2-001 ~ P2-009
- Milestone B（前�?MVP）：覆盖 FE-001 ~ FE-006（后续会�?TASKS 中追加）
- Milestone C（稳定性与交付）：覆盖 REL-001 ~ REL-xxx（后续会�?TASKS 中追加）
...
- Agent 在完成某里程碑的最后一个任务后，必须：
  1) 运行门禁并记录结�?
  2) �?TASKS.md 中为该里程碑写一段“里程碑验收记录”（命令 + 关键输出摘要�?
  3) 然后才可以开始下一里程碑的任务
- 未通过门禁不得推进里程碑，也不得更�?CURRENT.md 表示“已完成”�?
...
1) 一次只做一个任�?ID（禁止并行推进多个任务）
2) 必须先读：ai-docs/PROJECT.md / AGENTS.md / CONTEXT.md / CURRENT.md / TASKS.md
3) 必须“纵向切片”：先打通一条链路再扩展
4) 必须最小改动：禁止无关重构；优先新增文件而不是重�?
5) 必须可复现：每个任务都要给出验证命令（curl / scripts / pytest�?
6) 必须更新至少其一：README �?scripts（推�?scripts/dev_smoke.sh�?
7) 如需新增依赖、重�?schema 改动、改变目录结构：必须先提出并等待确认
...
**目标**
引入最小可用的“测验闭�?+ 用户画像”持久化结构�?

**交付**
- 新增/更新表（可按你的现有命名微调，但语义必须一致）�?
  - quizzes（一次测验的元信息）
  - quiz_questions（测验题目）
  - quiz_submissions �?quiz_attempts（提交与得分�?
...
- 能插入并查询一�?quiz + question + profile（脚本或临时验证代码均可�?
- README 增加迁移命令与验证方�?
...
- learner_profile 记录�?>= 1�?026-01-25�?

---

### [x] P2-003 接口�?quiz/generate（先 Easy-only 跑通）
**目标**
打通测验生成链路：检索资料片�?�?生成题目（只生成 Easy）→ 落库 �?返回结构�?JSON�?
...
- questions[] 每题必须包含�?
  ...
  - stem
  - options（仅 single 需要）
  - answer（结构化：single 为选项标识，judge �?true/false，short 为参考要�?参考答案）
  - explanation（基于资料片段的解析/要点�?
  - source_chunk_ids（至�?1 个）
  - related_concept（先用简单规则即可，如由 chunk 元数�?标题推断�?

**硬约�?*
- 必须“资料优先”：题目必须基于检索到�?chunk；若检索为空，返回 409 并提示“资料不�?索引未建�?
- 输出必须是结构化字段齐全（缺字段视为失败�?
- 允许沿用 Phase 1 �?MockLLM（本任务不强制接外部 LLM�?

**验收**
- �?sample.md 生成至少 5 �?Easy（可多次调用覆盖题型�?
- quizzes �?quiz_questions 落库可查
...
### [~] P2-004 接口�?quiz/submit（评�?+ 提交记录落库�?
**目标**
提交答案 �?对客观题判分 �?简答题给参考答�?要点（不强制自动评分）→ 落库 submission/attempt�?
...
**验收**
- 单�?判断正确判分
- 简答题不强行判对错，至少返回参考答�?要点与自评提�?
- 提交记录落库可查
- scripts/dev_smoke.sh 增加：生成→提交→输出结�?
...
**交付**
- /quiz/submit 中更�?concept_stats�?
  - correct_count / wrong_count / last_seen
- /profile/me 返回 weak_concepts（按 wrong_count �?wrong_rate 排序�?

**验收**
- 连续提交�?concept_stats 随之变化
- /profile/me 可看到弱项概念列表（至少能出�?1 条）
...
**建议实现（先稳后准）**
- ability_level：最�?N 道客观题正确率分�?
  - 初级 <50%，中�?50~80%，高�?>=80%
- frustration_score�?
  - 单次 accuracy 很低或连续错�?�?上升
  - 表现�?�?缓慢下降（每次提�?-1，最�?0�?

**验收**
- 两个不同用户/会话在多次提交后画像会分�?
- frustration_score 在连续低分时上升并可�?/profile/me 体现

---

### [ ] P2-007 难度控制�?quiz/generate 根据画像决定 difficulty_plan
**目标**
让测验生成真正“因人而异”：根据 learner_profile 输出难度配比并按 plan 生成（引�?Medium/Hard）�?

**硬规则（必须实现�?*
- 初级�?frustration_score 高：Hard=0 �?Easy>=80%
- 中级：Easy�?0% + Medium�?0%（Hard 可先 0 �?<=10%�?
- 高级：Easy�?0% + Medium�?0% + Hard�?0%
- 错题优先（weak_concepts）但不得突破难度约束

**交付**
- /quiz/generate 返回 difficulty_plan（写�?quizzes.difficulty_plan_json�?
- quiz_questions.difficulty �?plan 分配

**验收**
- 构造两个画像（初级 vs 高级）在同一资料下生成测验，difficulty_plan 明显不同
- 初级用户 Hard 数量�?0（可脚本验证�?

---

### [ ] P2-008 过难回调（防翻车体验�?
**目标**
当测验明显过难时自动“降�?+ 鼓励反馈 + 下次推荐更保守”�?

**最小策略（至少实现一种）**
- 提交后判断：
  - accuracy < 30% �?�?5 题错 >= 4（客观题）→
    - frustration_score 上升
    - feedback_text 包含鼓励 + 具体建议（先复习哪一块）
    - 设置 next_quiz_recommendation = "easy_first"
- 下一�?generate：如�?recommendation �?easy_first �?frustration 高，plan 自动保守
...
**验收**
- 你本地执�?dev_smoke.sh 全通过（或明确失败点）
- 输出包含：quiz_id、score/accuracy、profile 关键字段变化、difficulty_plan

---

## 五、后续（可选增强任务，完成当前队列后再添加�?
- ability_level �?theta（Elo/IRT-lite）能力估�?
- 更细粒度 concept 体系（章�?小节映射�?
