...
### [ ] FE-006 仪表盘详情页（逐题记录）
**目标**
在仪表盘中查看单次测验的逐题详情与错误原因，形成可复盘的记录视图。

**交付物**
- 后端新增最小详情接口（如 GET /quizzes/{quiz_id} 或 /quizzes/detail）
- 返回逐题详情（题干/题型/正确性/作答/参考答案/解析/来源）
- 前端新增详情页面/弹层，从 /dashboard 进入
- README 补充接口说明与用法

**验收**
- 从 /dashboard 点击记录可查看该次逐题详情
- 详情仅返回当前 session_id 关联的测验
- 端到端流程无需手动 curl

**依赖**
- FE-005
- quiz_questions / quiz_attempts 现有数据

**风险与回滚**
- 风险：返回体偏大
- 回滚：保留列表页，隐藏详情入口

### [ ] FE-007 学习路径增强（基于近期测验与薄弱点）
**目标**
提升学习路径的“可行动性”与“可解释性”，形成更接近产品体验的下一步建议。

**交付物**
- 路径规则增强：结合 weak_concepts + 最近测验相关概念
- 每一步附“推荐理由 + 来源链接（如文档/测验）”
- 若需要后端接口，新增最小 /learning/plan
- README 补充说明
...
**风险与回滚**
- 风险：路径质量波动
- 回滚：沿用当前 LearningPath Lite

### [ ] FE-008 资料组织（集合/标签/最近使用）
**目标**
支持多文档组织与快速回访，为长周期学习做准备。

**交付物**
- 文档集合/标签最小数据结构（可迁移）
- 最近使用/最近上传列表接口（如 /docs/recent）
- 前端资料管理页增加集合/标签/最近列表
- README 增加使用说明

**验收**
- 前端可查看最近文档列表并按标签筛选（最小版）
- 文档标签可添加/移除
- 端到端流程无需手动 curl

**依赖**
- documents/chunks 现有结构

**风险与回滚**
- 风险：引入迁移影响现有数据
- 回滚：保留原上传与索引流程，隐藏新入口

### [ ] FE-009 题目质量与解释增强（最小规则）
**目标**
提升题目质量与可解释性，降低重复与“难以追溯”的题目。

**交付物**
- 生成规则：避免同一 quiz 中题干/选项重复
- 解析规则：所有题必须包含 explanation + source_chunk_ids
- 失败回退：缺失时自动降级或重试（最小实现）
- README 说明质量约束
